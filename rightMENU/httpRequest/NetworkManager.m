       ...
       failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    _sessionManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/plain",@"text/javascript",nil];
    
   // NSString *userid = [UserHelpClass getInfomationFromNSUserdefaultWith:@"testUserid"];
//    if (userid==nil) {
//        [_sessionManager.requestSerializer setValue:@"" forHTTPHeaderField:@"userid"];
//    }else{
//        [_sessionManager.requestSerializer setValue:userid forHTTPHeaderField:@"userid"];
//    }
//    NSLog(@"%@",[NSString stringWithFormat:@"%@%@",TESTURL,url]);
//    NSLog(@"%@",parameters);
    //开始请求内容
    [_sessionManager GET:[NSString stringWithFormat:@"%@%@",TESTURL,url] parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        //如果需要填充进度内容，可以直接进行内容添加
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        successful(responseObject);
//        if (![url containsString:@"APP/API/Login/forgetpwd"]&&![url containsString:@"APP/API/User/exit"]) {
            if ([responseObject[@"code"]integerValue] == 401000||[responseObject[@"code"]integerValue] == 1006) {
               // [AlertUtils error:@"账号异常,请返回重新登录"];
//                [self.getCurrentVC.navigationController popToRootViewControllerAnimated:YES];
////                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"testUserid"];
...
//                }
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"phone"];
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"password"];
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"testUserid"];
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:JPUSHID];
//                [UserHelpClass removeUserInfoModel];
 //               self.flag = YES;
            }
//            else{
//                self.flag = NO;
...
//    }
    //开始请求内容
    [_sessionManager POST:[NSString stringWithFormat:@"%@%@",TESTURL,url] parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        NSLog(@"%@",[NSString stringWithFormat:@"%@%@",TESTURL,url]);
        //如果需要填充进度内容，可以直接进行内容添加
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        successful(responseObject);
//        if (![url containsString:ELogin]&&![url containsString:@"APP/API/Login/forgetpwd"]&&![url containsString:@"APP/API/User/exit"]) {
            if ([responseObject[@"code"]integerValue] == 401000||[responseObject[@"code"]integerValue]==1006) {
               // [AlertUtils error:@"账号异常,请重新登录"];
//                 [self.getCurrentVC.navigationController popToRootViewControllerAnimated:YES];
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"testUserid"];
//                LoginViewController *vc = [[LoginViewController alloc] init];
//                [[self getCurrentVC] presentViewController:vc animated:YES completion:^{
//                }];               // vc.outlogin = YES;
              //  vc.isDismissLast = YES;
//                if (!self.flag) {
//                    [[self getCurrentVC] presentViewController:vc animated:YES completion:nil];
//                }
//                [UserHelpClass saveInfomationToNSUserdefaultWitObj:@"" Key:@"phone"];
...
//    }
    ...
    [_sessionManager POST:[NSString stringWithFormat:@"%@%@",TESTURL,url] parameters:paramters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //对上传完文件的配置
        //获取当前时间（int 时间戳转换）
